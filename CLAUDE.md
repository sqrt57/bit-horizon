...
The Compose root composable is `App()` in `commonMain/.../App.kt`; both platform entry points call into it. The UI is built around four fixed vertical zones (telemetry metrics, log hub, infrastructure tiers, executive core/prestige button) as specified in `design/gdd.md`, each wired directly to a `GameState` held in `App()` via `remember { mutableStateOf(GameState()) }`. A `LaunchedEffect` drives the game loop by calling `tick()` every `TICK_DELTA_SECONDS` (100ms); tier purchases, manual scans, and Horizon Shift all go through `GameState`'s existing methods, so `App.kt` itself holds no game math.
...
`dataBitsPerSecond()` exposes the GDD's Prestige Modifier formula (`Total Bits/Sec = Base Bits/Sec × (1 + Tachyon Particles)`) as a queryable value for the UI's Zone 1 readout, rather than making callers reimplement it. `manualScan()` grants a flat `MANUAL_SCAN_YIELD` Data Bits for the Executive Core's manual-tap action — the GDD doesn't specify this value (see `design/backlog.md`), so it's a placeholder pending a balance pass.
